...
Status addGenericAndroidFunctions(MonitorFfs* monitorFfs, uint64_t functions, bool* ffsEnabled,
    ...
    if ((functions & GadgetFunction::NCM) != 0) {
        ...
        std::string ncmFunction = GetProperty(kVendorNcmConfig, "");
        if (ncmFunction != "") {
            if (linkFunction(ncmFunction.c_str(), (*functionCount)++)) return Status::ERROR;
        } else {
            // link gsi.ncm for older pixel projects
            if (linkFunction("gsi.ncm", (*functionCount)++)) return Status::ERROR;
        }
    }
